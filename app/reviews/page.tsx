...
export default function ReviewsPage() {
  ...
  return (
    <AdminShell title="리뷰" subtitle={`total: ${total === null ? '—' : total}`}>
      {err ? (
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50 text-left text-gray-600">
            <tr>
              ...
              <th className="px-3 py-2">caregiverId</th>
          ...
          <tbody>
            {rows.map((r: any) => (
              <tr key={String(r.id)} className="border-t">
                ...
                <td className="px-3 py-2">{String(r.caregiverId ?? r.caregiver_id ?? '—')}</td>
                <td className="px-3 py-2">{String(r.matchId ?? r.match_id ?? '—')}</td>
